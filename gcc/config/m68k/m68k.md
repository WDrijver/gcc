...
;;  //else if (TARGET_68080 && MEM_P (operands[0]) && GET_CODE (operands[1]) == CONST_INT && INTVAL (operands[1]) == 0)   // APOLLO 68080 (ALINE = Disabled)
;;  //  {
;;	//	return "clr%.q %0";		
;;	//  }
  return output_move_double (operands);
...
;; "(!ISA_HAS_MVS_MVZ) || TARGET_68080) - APOLLO 68080 (ALINE = Disabled)

(define_split
  [(set (match_operand 0 "register_operand" "") (zero_extend (match_operand 1 "nonimmediate_src_operand" "")))]
  "(!ISA_HAS_MVS_MVZ) && reload_completed && reg_mentioned_p (operands[0], operands[1])"
  [(set (strict_low_part (match_dup 2)) (match_dup 1)) (set (match_dup 0) (match_op_dup 4 [(match_dup 0) (match_dup 3)]))]
{
  operands[2] = gen_lowpart (GET_MODE (operands[1]), operands[0]);
...
;; "TARGET_COLDFIRE || TARGET_68080" - APOLLO 68080 (ALINE = Disabled)
  ...
  "!TARGET_COLDFIRE"
{
  ...
  if (DATA_REG_P (operands[0]))
	{
    if (TUNE_68080) return "dbra.l %0,%l1";                 // APOLLO 68080
    return "dbra %0,%l1\;clr%.w %0\;subq%.l #1,%0\;jcc %l1";
	}
  if (GET_CODE (operands[0]) == MEM) return "subq%.l #1,%0\;jcc %l1";
...
;; (TARGET_68080 || GET_MODE_SIZE(GET_MODE(operands[4])) == 2) - APOLLO 68080 (ALINE = Disabled)
...
;;   		(match_dup 1))
;;   ]
;;  "TARGET_68080 && current_pass->static_pass_number >= 2 && 
;;   0 == INTVAL (operands[1]) && 
;;   (operands[4] = SET_DEST(PATTERN(insn))) && !operands[4]->volatil &&
